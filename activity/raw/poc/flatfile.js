let fs = require("fs"); // fs is a NodeJS module/package
...
function readContent(src) {
    return fs.readdirSync(src); // synchronously
}
...
//     }
// }
function viewTree(src, indent) { // indent is the space 
    let isFile = isFileOrNot(src);
    if (isFile == true) {
        console.log(indent, path.basename(src), "*"); // path.basename returns the last portion of a path
    }
    else {
        // print
